...
def get_wave_y(x, t):
    # 波の先端 x = v*t より右側 (x > v*t) では変位0
    return np.where(x <= v * t, -A * np.sin(k * x - omega * t), 0.0)

# 初期状態の波のプロット
line, = ax.plot(x, get_wave_y(x, t), lw=2, color='blue')
...
ax_play  = plt.axes([0.10, 0.05, 0.14, 0.075])
ax_stop  = plt.axes([0.26, 0.05, 0.14, 0.075])
ax_prev  = plt.axes([0.42, 0.05, 0.14, 0.075])
ax_next  = plt.axes([0.58, 0.05, 0.14, 0.075])
ax_reset = plt.axes([0.74, 0.05, 0.14, 0.075]) # 追加: リセットボタン
...
def reset(event):
    global t, is_playing
    is_playing = False  # リセット時は自動再生を停止
    t = 0.0
    line.set_ydata(get_wave_y(x, t))
    time_text.set_text(f'Time = {t:.2f} s')
    fig.canvas.draw_idle()
